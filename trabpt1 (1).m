...
for linha=1:linhas %linha 1 é 1-2, linha 2 é 2-3, linha 3 é 3-5 e linha 4 é 5-4.
        ...
        Y(a1,a1) = Y(a1,a1)+1/(r1(linha)+j*x1(linha));
        Y(a1,a2) = Y(a1,a2)-1/(r1(linha)+j*x1(linha));
        Y(a2,a1) = Y(a2,a1)-1/(r1(linha)+j*x1(linha));
        Y(a2,a2) = Y(a2,a2)+1/(r1(linha)+j*x1(linha));

end
    Y(1,1) = Y(1,1)+1/(rsu1+j*xsu1);
    Y(5,5) = Y(5,5)+j*xg1; 
        Y2 = Y;
        Z2 = inv(Y2);
        Y1 = Y;
...
for linha=1 : linhas
        ...
        Y(a1,a1) = Y(a1,a1)+1/(r2(linha)+j*x2(linha));
        Y(a1,a2) = Y(a1,a2)-1/(r2(linha)+j*x2(linha));
        Y(a2,a1) = Y(a2,a1)-1/(r2(linha)+j*x2(linha));
        Y(a2,a2) = Y(a2,a2)+1/(r2(linha)+j*x2(linha));

end
    Y(1,1) = Y(1,1)+1/(rsu2+j*xsu2);
    Y(5,5) = Y(5,5)+j*xg2; 
        Y2 = Y;
        Z2 = inv(Y2);
...
for linha=1 : linhas
        ...
        Y(a1,a1) = Y(a1,a1)+1/(r0(linha)+j*x0(linha));
        Y(a1,a2) = Y(a1,a2)-1/(r0(linha)+j*x0(linha));
        Y(a2,a1) = Y(a2,a1)-1/(r0(linha)+j*x0(linha));
        Y(a2,a2) = Y(a2,a2)+1/(r0(linha)+j*x0(linha));

end

    Y(1,1) = Y(1,1)+1/(rsu0+j*xsu0);
    Y(2,2) = 1/(r0(1)+j*x0(1));
    Y(5,5) = Y(5,5)+j*xg0; 
